...
## PARKED HERE (2026-06-12). Resume notes

- **Repo:** `git@github.com:dino-trustap/trustap_index.git`, branch `main`, everything committed and pushed. Module path stays `github.com/trustap/trustap_index` on purpose (eventual org home; harmless mismatch for an application).
- **Next task, explicitly agreed:** the **ACP `checkout_sessions` adapter**: one build covers BOTH ChatGPT and Copilot (Copilot runs on ACP verbatim). Reference implementation: the frozen Python MVP `~/Projects/Agentic commerce/app/adapters/acp/` (mappers, session state machine, POST-based update, complete returns session-with-order; map complete -> our checkout -> Trustap pay_url as `continue_url`). After that: UCP checkout (Universal Cart), merchant-side inventory sync, Shopify ingestion connector.
- **Discovery vs checkout status:** discovery surfaces exist for ALL agents (ACP/Copilot/UCP manifests + feeds, GMC CSV, JSON-LD pages); transacting is via the protocol-neutral native `POST /api/checkouts` only; no protocol-native checkout yet.
- **Machine-local state (NOT in git):** stage credentials live only in `configs/api.yaml` on this machine (recreate from `api.sample.yaml` if cloned elsewhere: demo merchant API key + seller sub, `trustap_oauth` client_id/secret for future consent endpoints, webhook auth pair `test`/`test`). Postgres data (demo products, checkouts, webhook events) lives in the `trustap-index-db` docker volume; `docker compose up -d` brings it back.
- **Webhook URL is dead until redone:** the cloudflared quick tunnel was ephemeral and is stopped. Next session: start a new tunnel (`cloudflared tunnel --url http://localhost:8090`), update `public_base_url` in `configs/api.yaml`, and Dino updates the webhook URL in the Trustap internal dashboard (auth pair stays `test`/`test`). Permanent deployment home (the `index.trustap.com` idea) is still undecided.
- **Standing open questions:** iframe vs own payment UI; shipping (Trustap API has Shippo endpoints); merchant onboarding/consent flow; what batch-cancelled stage transactions 35065/35066 (ask internally); variant support for products.
